...
int* solve(int arr_count, int* arr, int queries_count, int* queries, int* result_count) {
    int left[arr_count], right[arr_count];
    int stack[arr_count];
    int top = -1;
    ...
    for(int len = arr_count - 1; len >= 1; len--){
        if (minmax[len] > minmax[len + 1])
            minmax[len] = minmax[len + 1];
    }
    
    *result_count = queries_count;
    int* res = malloc(queries_count * sizeof(int));
    for (int i = 0; i < queries_count; i++)
        res[i] = minmax[queries[i]];
        return res; 
