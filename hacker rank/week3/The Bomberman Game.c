 ...
 */
 ...
 void explode(char** src, char** dest, int rows, int cols){
    static int dr[] = {-1, 1, 0, 0};
    static int dc[] = {0, 0, -1, 1};
    for (int i = 0; i < rows; i++){
        memset(dest[i], 'O', cols);
    }
    ...
    for(int i = 0; i < rows; i++) dest[i][cols] ='\0';
 }
...
char** bomberMan(int n, int grid_count, char** grid, int* result_count) {
    ...
    *result_count = rows;
    ...
    for (int i = 0; i < rows; i++){
        output[i] = malloc((cols + 1) * sizeof(char));
    }
    
    if (n == 1){
        for (int i = 0; i < rows; i++){
            strcpy(output[i], grid[i]);
        }
        return output;
    }
    ...
    if ( n % 2 == 0){
        for (int i = 0; i < rows; i++){
            memset(output[i], 'O', cols);
            output[i][cols] = '\0';
        }
        return output;
    }
    
    char** state1 = malloc(rows * sizeof(char*));
    char** state2 = malloc(rows * sizeof(char*));
    for (int i =0; i < rows; i++){
        state1[i] = malloc((cols + 1) * sizeof(char));
        state2[i] = malloc((cols + 1) * sizeof(char));
        strcpy(state1[i], grid[i]);
    }
    
    explode(state1, state2, rows, cols);
    
    if (n % 4 == 3){
        for (int i = 0; i < rows; i++) strcpy(output[i], state2[i]);
    } else {
        explode(state2, state1, rows, cols);
        for (int i = 0; i < rows; i++) strcpy(output[i], state1[i]);
    }
